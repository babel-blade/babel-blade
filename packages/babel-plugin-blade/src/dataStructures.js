...
export class RazorData {
	constructor({ args = null, name = null, type = null, fragmentType = null }) {
		...
		if (type === 'fragment' && !fragmentType) throw new Error('fragments must come with a fragmentType');
	...
	print() {
		...
		let accumulators = Object.keys(fields).map(key => fields[key].print(indent, fragments));
...
export class BladeData {
	constructor({ name = null, args = null, fragments = [], directive = null }) {
		...
		this._alias = args && `${this._name}_${hashCode(JSON.stringify(args))}`;
	...
	print(indent, fragments) {
		...
		if (fields.length || this._fragments.length) {
			...
			this._fragments.forEach(frag => {
				TemplateLiteral.addStr(`${indent}  ...${frag.getFragmentData().name}\n`);
			});
...
function generateFields(obj, references, indent = '  ') {
	return (
		...
		Object.keys(obj)
			.map(key => {
				let name = key;
				if (obj[key] && obj[key] && Object.keys(obj[key]).length && obj[key].isReference()) {
...
function maybeGetSimpleString(Literal) {
	...
	if (Literal.type === 'TemplateLiteral' && !Literal.expressions.length && Literal.quasis.length === 1)
